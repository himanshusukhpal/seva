...
export class Base64Service {
  ...
  _utf8_encode(inputString: string) {
  ...
  _utf8_decode(utftext: string) {
    let inputString = '';
    let i = 0;
    let c = 0; const c1 = 0; let c2 = 0; let c3;
    while (i < utftext.length) {
      ...
      if (c < 128) {
        ...
        inputString += String.fromCharCode(((c & 31) << 6) | (c2 & 63));
        i += 2;
      } else {
        c2 = utftext.charCodeAt(i + 1);
        c3 = utftext.charCodeAt(i + 2);
        inputString += String.fromCharCode(((c & 15) << 12) | ((c2 & 63) << 6) | (c3 & 63));
        i += 3;
      }
    }
    return inputString;
